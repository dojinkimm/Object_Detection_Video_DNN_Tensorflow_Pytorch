...
import cv2
from pytorch_v1.util import convert2cpu as cpu
from pytorch_v1.util import predict_transform
...
class Upsample(nn.Module):
    def __init__(self, stride=2):
        ...
        def forward(self, x):
            ...
            ws = stride
            hs = stride
            x = x.view(B, C, H, 1, W, 1).expand(B, C, H, stride, W, stride).contiguous().view(B, C, H * stride,
                                                                                              W * stride)
            return x
